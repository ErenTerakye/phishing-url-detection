...
FIG_DPI = 150
FIG_KWARGS = dict(dpi=FIG_DPI, bbox_inches='tight')
...
def plot_confusion_matrix(y_true, y_pred, model_name, save_path):
    ...
    fig.savefig(save_path, **FIG_KWARGS)
    plt.close(fig)
...
def compare_models_table(results_dict):
    ...
    df[numeric_cols] = df[numeric_cols].applymap(lambda x: round(x, 4) if x is not None else None)
